...
- 세로쓰기라고 판단될 경우, 일부 특수문자를 새로쓰기에 맞게 변경합니다. 예를 들어, '()'은 '︵︶'으로, '.'는 '︒'으로, '[]'은 '﹁﹂'으로 바꿉니다. 15개 특수문자에 대해서 규칙을 만들었고 새로운 특수문자가 나타날 경우 규칙을 추가하기로 하였습니다.
...
- 언어별로 줄의 시작 또는 끝에 올 수 없는 문자와 같은 규칙이 있는데 [5], 이에 위배되지 않도록 정교한 문자열 처리 알고리즘을 개발했습니다.
...
### (3) 텍스트 영역 인식
- 기존에는 텍스트가 렌더링될 수 있는 영역을 바운딩 박스 내로 한정했습니다. 이 방법은 가장 간단하지만, 번역문이 원문보다 긴 경우 폰트 크기를 더 작게 만듭니다.
- 따라서 바운딩 박스를 벗어나 원본에 더 근접한 폰트 크기를 사용할 수 있는 알고리즘을 개발했으나 [2], 서비스에 적용하지는 못했습니다.
- 알고리즘:
    - 렌더링할 텍스트가 다른 텍스트나 선 등과 겹치지 않도록 제한하기 위해 마스크를 생성하고 이를 활용합니다.
    - 바운딩 박스가 차지하는 영역을 넘어 텍스트가 렌더링 될 수 있도록, 원본 폰트 크기보다 작은 선에서 폰트 크기를 순차적으로 증가시킵니다.
    - 앞서 추출된 폰트 크기의 0.5배에서 시작하여, 원본 폰트 크기의 0.6배 → 0.7배 → 0.8배 → 0.9배 → 1.0배 순으로 총 6개의 폰트 크기를 가지고 텍스트 렌더링을 시도합니다.
    - 마스크와 렌더링한 텍스트가 겹치는지 확인합니다. 겹치지 않으면 해당 폰트 크기를 출력합니다.
    - 이때 텍스트가 길 경우 가능한 줄바꿈의 경우의 수는 매우 많으므로 모든 경우에 대해 텍스트를 렌더링해보는 것은 계산량이 너무 많습니다.
    - 따라서 텍스트를 서브워드 단위로 분리하고 이것을 하나씩 붙여가면서 원래의 텍스트에 가깝게 조금씩 문자열을 만들고 완성되어가는 문자열을 가지고 렌더링을 시도합니다.
    - 중간에 겹침이 발생하면 줄바꿈을 삽입하고 이어서 서브워드를 이어 붙여 나갑니다. 겹침이 두 번 연속 해당 폰트 크기로는 렌더링이 불가하므로 바로 직전 단계의 폰트 크기를 출력합니다.
- 결과:
    |텍스트 영역 인식 적용 전후|
    |-|
...
# 4. References
- [1] [Character Region Awareness for Text Detection](https://arxiv.org/abs/1904.01941)
