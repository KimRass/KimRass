...
- 성격:
    - 매일 공부하는 자세:
        ...
        - 업무를 수행하는 데 있어서 갖추어야하는 능력이 있다면 악착같이 파고들어 프로젝트를 성공적으로 이끎.
        - 데이터 분석과 머신 러닝을 독학하여 아파트 실거래가 데이터 분석을 리드.
        - Tableau와 SQL을 깊게 공부하여 EIS를 성공적으로 구축.
        - scene text detection, image inpainting 등의 digital image processing을 공부하여 scene text removal 모델 개발.
    - 커뮤니케이션 능력:
        제품 또는 서비스를 개선하기 위한 방안에 대해 토론하는 것을 좋아하며 다른 사람의 이야기를 경청함.
...
|알고리즘|• '프로그래머스': 1434점<br>• ['Baekjoon Online Judge' AC rating: Gold III 1115.](https://solved.ac/en/profile/rmx1000)||
|PyTorch|• 주어진 데이터를 원하는 태스크에 맞게 가공하고 dataloader 설계 가능.<br>• [딥 러닝 논문을 읽고 모델 아키텍처, 등을 from scratch로 구현하고 학습시킬 수 있으며 각종 시각화, 실험?](#1-딥-러닝-논문-구현-경험)|• `accelerate` 라이브러리를 통해 다수의 GPU를 활용한 데이터 분산 학습 경험.|
|AWS|||
|Markdown|모든 문서를 마크다운으로 작성하는 습관||
|AutoCAD|SolidWorks로 그린 3D 도면을 가져와 2D로 변환한 후 도면 작성법에 따라 후처리?||
|SolidWorks|2D 도면을 보고 3D 물체를 그릴 수 있음|
|스프레드시트|각종 함수와 필터를 사용하여 자유자재로 데이터 집계.|• 이미지 데이터 수집 프로젝트에서 Google Sheets를 활용한 데이터 입력 프로세스에 있어서 드롭다운 메뉴와 문자열 관련 함수를 활용하여 인적오류를 최소화.|
|파워포인트||• 워드로 작성된 기계설비 시공지침 전체를 파워포인트로 재작성.<br>• 아파트 실거래가 데이터 분석 프로젝트 보고서 작성.<br>• video super-resolution 알고리즘을 설명하는 자료 작성.|
...
- 국내 3대 백화점 브랜드 및 다수의 지자체와 메뉴 이미지 번역 서비스 계약 체결.
...
#### 수행 업무
- [**data QC 알고리즘 개발**](https://github.com/KimRass/KimRass/tree/main/Flitto/Data-Collection)
...
#### 참여 인원
- 본인 외 태블로 개발자 2명, data mart 개발자 4명 등.
