...
- [**메뉴 이미지 생성 자동화 기능 개발**:](https://github.com/KimRass/KimRass/tree/main/Flitto/Place-Translation/Menu-Image-Generator)
    - 엑셀 파일 입력 받아 메뉴 이미지를 자동으로 생성하는 기능 개발.
...
#### 수행 업무
- 프로젝트 총괄하여 개발의 방향성과 일정 관리.
- 각 부서의 KPI를 대시보드에 효과적으로 표한하기 위해 현업 담당자와 협의하여 요구사항을 수집하고 이를 반영.
- 대시보드 간의 일관성을 유지하기 위해 통일된 디자인 양식 수립하고 이를 적용.
- SQL 쿼리를 작성하여 데이터를 집계하고 이를 Tableau 차트와 비교 검증.
- 대시보드에 사용된 모든 테이블과 Tableau 알고리즘 점검.
- 대시보드의 모든 구성 요소가 정확한 데이터를 반영하도록 정기적으로 점검.
...
- 하나의 아파트 단지를 제외한 나머지 단지의 실거래가 데이터를 모델에 학습시킨 후 해당 단지의 실거래가를 예측하도록 하는 leave-one-out cross-validation을 사용.
